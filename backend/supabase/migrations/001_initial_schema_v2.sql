...
-- =====================================================
-- COURT AVAILABILITY & BOOKING SLOTS
-- =====================================================

-- Court availability time slots to prevent double-booking
...
CREATE INDEX idx_court_availabilities_court ON court_availabilities(court_id);
CREATE INDEX idx_court_availabilities_time ON court_availabilities(start_time, end_time);
CREATE INDEX idx_court_availabilities_reserved ON court_availabilities(court_id, is_reserved);

COMMENT ON TABLE court_availabilities IS 'Time slot availability tracking to prevent double-booking';

-- =====================================================
-- DYNAMIC PRICING & DISCOUNTS
-- =====================================================

-- Discount rules for multi-day, group, and loyalty discounts
CREATE TABLE discount_rules (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  venue_id uuid REFERENCES venues(id) ON DELETE CASCADE,
  name varchar(100) NOT NULL,
  description text,
  discount_type varchar(20) NOT NULL CHECK (discount_type IN ('multi_day', 'group', 'loyalty', 'early_bird', 'custom')),
  discount_value numeric(5,2) NOT NULL,
  discount_unit varchar(10) NOT NULL CHECK (discount_unit IN ('percent', 'fixed')),
  min_days int CHECK (min_days > 0),
  min_players int CHECK (min_players > 0),
  min_bookings int CHECK (min_bookings > 0),
  advance_days int CHECK (advance_days > 0),
  is_active boolean NOT NULL DEFAULT true,
...
COMMENT ON TABLE discount_rules IS 'Configurable discount rules for various booking scenarios';
...
CREATE TABLE holiday_pricing (
  ...
  fixed_surcharge numeric(9,2) DEFAULT 0,
  is_active boolean NOT NULL DEFAULT true,
  created_at timestamptz NOT NULL DEFAULT now(),
  CHECK (end_date >= start_date),
  CHECK (price_multiplier >= 0 OR fixed_surcharge >= 0)
);
...
COMMENT ON TABLE holiday_pricing IS 'Holiday and special event pricing adjustments';
...
-- Audit logs for platform-wide activity tracking
...
COMMENT ON TABLE audit_logs IS 'System-wide activity and change tracking';

-- =====================================================
-- ENHANCED RATING SYSTEM
-- =====================================================

-- Venue owner responses to court ratings
CREATE TABLE rating_responses (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  rating_id uuid NOT NULL REFERENCES court_ratings(id) ON DELETE CASCADE,
  venue_id uuid NOT NULL REFERENCES venues(id) ON DELETE CASCADE,
...
CREATE INDEX idx_rating_responses_venue ON rating_responses(venue_id);

COMMENT ON TABLE rating_responses IS 'Venue owner responses to court reviews';
...
COMMENT ON TABLE rating_helpful_votes IS 'User votes on review helpfulness';
...
CREATE TABLE notification_preferences (
  ...
  queue_notifications boolean NOT NULL DEFAULT true,
...
CREATE TRIGGER update_court_availabilities_updated_at BEFORE UPDATE ON court_availabilities
  FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

CREATE TRIGGER update_discount_rules_updated_at BEFORE UPDATE ON discount_rules
  FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

CREATE TRIGGER update_rating_responses_updated_at BEFORE UPDATE ON rating_responses
  FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
...
ALTER TABLE holiday_pricing ENABLE ROW LEVEL SECURITY;
...
CREATE POLICY "Court availabilities are viewable by everyone" ON court_availabilities
  FOR SELECT USING (true);

CREATE POLICY "Venue owners can manage court availabilities" ON court_availabilities
  FOR ALL USING (
    EXISTS (
      SELECT 1 FROM courts
      JOIN venues ON courts.venue_id = venues.id
      WHERE courts.id = court_availabilities.court_id
      AND venues.owner_id = auth.uid()
    )
  );

-- Discount rules policies
CREATE POLICY "Active discount rules are viewable by everyone" ON discount_rules
  FOR SELECT USING (is_active = true);

CREATE POLICY "Venue owners can manage their discount rules" ON discount_rules
  FOR ALL USING (
    EXISTS (
      SELECT 1 FROM venues
      WHERE venues.id = discount_rules.venue_id
      AND venues.owner_id = auth.uid()
    )
  );

-- Holiday pricing policies
CREATE POLICY "Active holiday pricing is viewable by everyone" ON holiday_pricing
  FOR SELECT USING (is_active = true);

CREATE POLICY "Venue owners can manage their holiday pricing" ON holiday_pricing
  FOR ALL USING (
    EXISTS (
      SELECT 1 FROM venues
      WHERE venues.id = holiday_pricing.venue_id
      AND venues.owner_id = auth.uid()
    )
  );
...
-- Audit logs policies (admin only - implement custom logic)
CREATE POLICY "Users can view own audit logs" ON audit_logs
  FOR SELECT USING (auth.uid() = user_id);

-- Rating responses policies
CREATE POLICY "Rating responses are viewable by everyone" ON rating_responses
  FOR SELECT USING (true);

CREATE POLICY "Venue owners can create responses" ON rating_responses
  FOR INSERT WITH CHECK (
    EXISTS (
      SELECT 1 FROM venues
      WHERE venues.id = rating_responses.venue_id
      AND venues.owner_id = auth.uid()
    )
  );

CREATE POLICY "Venue owners can update own responses" ON rating_responses
  FOR UPDATE USING (auth.uid() = responder_id);

-- Rating helpful votes policies
CREATE POLICY "Users can view rating votes" ON rating_helpful_votes
  FOR SELECT USING (true);

CREATE POLICY "Users can vote on ratings" ON rating_helpful_votes
  FOR INSERT WITH CHECK (auth.uid() = user_id);
