...
export async function joinQueue(sessionId: string) {
  ...
  try {
    ...
    const runMissingUpdatedAtFallback = async () => {
      console.warn(
        '[joinQueue] ⚠️ Falling back to app-level join logic due to missing updated_at column'
      );
    ...
    if (rpcError) {
      ...
      if (
        rpcError.message?.includes(
          'column "updated_at" of relation "queue_participants" does not exist'
        )
      ) {
