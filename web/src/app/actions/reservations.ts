...
export async function getCourtDownPaymentPercentageAction(courtId: string) {
  ...
  const rawPercentage =
    courtMetadata?.down_payment_percentage ?? venueMetadata?.down_payment_percentage ?? 20;
  ...
  return {
    ...
    percentage,
    source:
      courtMetadata?.down_payment_percentage !== undefined
        ? 'court'
        : venueMetadata?.down_payment_percentage !== undefined
          ? 'venue'
          : 'default',
  };
}
...
export async function getCourtDownPaymentPercentagesAction(courtIds: string[]) {
  ...
  const maxPercentage =
    Object.values(percentages).length > 0 ? Math.max(...Object.values(percentages)) : 20;
...
export async function getAvailableTimeSlotsAction(
  ...
  const openingHours = (court.opening_hours || venueData?.opening_hours) as Record<
    string,
  ...
  console.log(
    `[Availability Check] Found ${reservations.length} reservations and ${queueSessions.length} queue sessions`
  );

  if (reservations.length > 0) {
    console.log(
      '[Availability Check] Reservations:',
      reservations.map((r) => ({
        start: r.start_time,
        end: r.end_time,
        status: r.status,
  ...
  if (allBookedSlots && allBookedSlots.length > 0) {
    for (const reservation of allBookedSlots) {
      try {
        ...
        const startHourStr = startTime.toLocaleString('en-US', {
          hour: 'numeric',
          hour12: false,
          timeZone: 'Asia/Manila',
        });
        const endHourStr = endTime.toLocaleString('en-US', {
          hour: 'numeric',
          hour12: false,
          timeZone: 'Asia/Manila',
        });
    ...
    for (const reservation of allBookedSlots) {
      try {
        ...
        for (const slot of allSlots) {
          ...
          const slotStartTS = new Date(
            dateOnlyString + `T${h.toString().padStart(2, '0')}:00:00+08:00`
          ).getTime();
          ...
          if (resStartTS < slotEndTS && resEndTS > slotStartTS) {
            ...
            console.log(
              `[Availability Check] Marking ${slot.time} as unavailable due to overlap with ${reservation.start_time} - ${reservation.end_time}`
            );
          }
        }
      } catch (error) {
...
export async function getVenueDailyAvailabilitySummaryAction(params: {
  ...
  if (reservationsResult.error) {
    console.error(
      '[getVenueDailyAvailabilitySummaryAction] Reservations query failed:',
      reservationsResult.error
    );
  }
  if (queueSessionsResult.error) {
    console.error(
      '[getVenueDailyAvailabilitySummaryAction] Queue sessions query failed:',
      queueSessionsResult.error
    );
  }
  ...
  for (const reservation of reservationsResult.data || []) {
    ...
    bookedByCourt[reservation.court_id].push({
  ...
  for (const queueSession of queueSessionsResult.data || []) {
    ...
    bookedByCourt[queueSession.court_id].push({
      start: queueSession.start_time,
      end: queueSession.end_time,
    });
  }
...
export async function getSameTimeBookingEligibleDatesAction(params: {
  ...
  const uniqueSelections = Array.from(
    new Map(
      selections.map((selection) => [
        `${selection.courtId}|${selection.startTime}|${selection.endTime}`,
        selection,
      ])
    ).values()
  );
  ...
  if (reservationsResult.error) {
    console.error(
      '[getSameTimeBookingEligibleDatesAction] Reservations query failed:',
      reservationsResult.error
    );
  }
  if (queueSessionsResult.error) {
    console.error(
      '[getSameTimeBookingEligibleDatesAction] Queue sessions query failed:',
      queueSessionsResult.error
    );
  }
  ...
  for (const reservation of reservationsResult.data || []) {
    ...
    eventsByCourt[reservation.court_id].push({
      start: reservation.start_time,
      end: reservation.end_time,
    });
  }
  for (const queueSession of queueSessionsResult.data || []) {
    ...
    eventsByCourt[queueSession.court_id].push({
      start: queueSession.start_time,
      end: queueSession.end_time,
    });
  }
...
export async function validateBookingAvailabilityAction(data: {
  ...
  for (let i = 0; i < recurrenceWeeks; i++) {
    for (const dayIndex of uniqueSelectedDays) {
      ...
      const slotStartTime = new Date(
        initialStartTime.getTime() + i * 7 * 24 * 60 * 60 * 1000 + dayOffset * 24 * 60 * 60 * 1000
      );
  ...
  for (let i = 0; i < targetSlots.length - 1; i++) {
    ...
    if (current.end.getTime() > next.start.getTime()) {
      return {
        available: false,
        error: 'Internal conflict: Generated slots overlap with each other.',
      };
    }
  }
  ...
  for (const slot of targetSlots) {
    ...
    if (slot.start.getTime() < Date.now() - 60000) {
      ...
      return {
        ...
        error: `Cannot book a time in the past: ${dateStr}`,
      };
    }
    ...
    if (slotStartMinutes < openMinutes || slotEndMinutes > closeMinutes) {
      ...
      return {
        available: false,
        conflictDate: dateStr,
        error: `Venue is closed at ${timeStr} on ${dayName}s (Open: ${dayHours.open} - ${dayHours.close})`,
      };
    }
    ...
    if (queueConflicts.data && queueConflicts.data.length > 0) {
      ...
      return {
        available: false,
        conflictDate: dateStr,
        error: `Queue Session conflict on ${dateStr}`,
      };
    }

    const realConflicts =
      reservationConflicts.data?.filter((conflict) => {
...
export async function createMultiCourtReservationsAction(data: {
  ...
  if (sortedItems.length > 0) {
    ...
    for (let i = 1; i < sortedItems.length; i++) {
      ...
      const sameDay =
        new Date(current.startTimeISO).toDateString() ===
        new Date(next.startTimeISO).toDateString();
      const consecutive = current.endTimeISO === next.startTimeISO;
      const samePayment =
        current.paymentType === next.paymentType && current.paymentMethod === next.paymentMethod;
  ...
  const uniqueDates = new Set(
    itemsToProcess.map((item) => new Date(item.startTimeISO).toDateString())
  );
...
export async function cleanupOldPendingReservationsAction(olderThanMinutes: number = 5) {
  ...
  console.log(
    `Found ${oldReservations.length} old pending reservations to clean up:`,
    oldReservations
  );
