...
import {
  processChargeableSourceAction,
  processPaymentByReservationAction,
  ...
  useEffect(() => {
    ...
    async function processPayment() {
      ...
      try {
        ...
        if (sourceId) {
          ...
          if (!result.success) {
            ...
            if (!retryResult.success) {
              setError(
                retryResult.error ||
                  'Failed to process payment. Please contact support if payment was deducted.'
              );
            } else {
        ...
        if (reservationId) {
          console.log(
            '[PaymentSuccessPage] No source_id, using reservation-based processing:',
            reservationId
          );
          ...
          if (!result.success && result.status === 'pending_payment') {
            ...
            if (!retryResult.success && retryResult.error) {
              ...
              console.warn(
                '[PaymentSuccessPage] Payment may still be processing:',
                retryResult.error
              );
            } else if (
              retryResult.success &&
              (retryResult.status === 'confirmed' || retryResult.status === 'partially_paid')
            ) {
              await clearCartAfterConfirmedPayment();
            }
