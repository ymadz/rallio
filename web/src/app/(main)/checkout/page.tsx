...
export default function CheckoutPage() {
  ...
  const isMultiCourt =
    effectiveCart.length > 1 || (effectiveCart[0]?.courts && effectiveCart[0].courts.length > 1);
  ...
  const totalCartAmount = effectiveCart.reduce((sum, item) => {
    ...
    const totalHourlyRate =
      item.courts && item.courts.length > 0
      ...
      if (paymentMethod === 'cash') {
        ...
        if (isDownPaymentRequired) {
          const minimumDownPayment =
            Math.round(finalTotal * ((downPaymentPercentage ?? 20) / 100) * 100) / 100;
          if (
            customDownPaymentAmount !== undefined &&
            customDownPaymentAmount > 0 &&
            customDownPaymentAmount < minimumDownPayment
          ) {
            return false;
          }
        }
      ...
      <div className="max-w-7xl mx-auto px-6 py-8">
        <div className="grid lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-6">
                    ...
                    <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between mb-4">
                      <div>
                        ...
                        <p className="text-sm text-gray-600">
                          {displayCart.length} court slots will be processed in one checkout.
                        </p>
                      </div>
                      <div className="grid grid-cols-3 gap-2 text-center">
                        ...
                        <div
                          ...
                          title={`${totalCartHours} duration x ${displayCart.length} courts`}
                        >
                          <p className="text-[11px] uppercase tracking-wide text-gray-500">Hours</p>
                          <p className="text-sm font-semibold text-gray-900">
                            {totalCartHours *
                              (isMultiCourt &&
                              effectiveCart[0]?.courts &&
                              effectiveCart[0].courts.length > 1
                                ? effectiveCart[0].courts.length
                                : 1)}
                          </p>
                        </div>
                        <div className="rounded-lg border border-gray-200 bg-gray-50 px-3 py-2">
                ...
                <div className="bg-white border border-gray-200 rounded-xl p-6">
                  ...
                  {isMultiCourt && (
                    <div className="mb-3 p-3 bg-amber-50 border border-amber-200 rounded-lg text-sm text-amber-800">
                      Showing aggregate totals for {displayCart.length} court items. Per-item
                      pricing appears in Booking Summary.
                    </div>
                  )}
                  <div className="space-y-3">
                    ...
                    <div className="flex justify-between py-3 border-b border-gray-100">
                      ...
                      <div className="text-right">
                        <span className="font-medium text-gray-900">
                          ₱{bookingData.hourlyRate.toFixed(2)}
                        </span>
                        {bookingData.courts && bookingData.courts.length > 1 && (
                          <span className="block text-[10px] text-gray-500">
                            × {bookingData.courts.length} courts = ₱
                            {(bookingData.hourlyRate * bookingData.courts.length).toFixed(2)}/hr
                            total
                          </span>
                        )}
                      </div>
                      ...
                      <div className="flex justify-between py-3 border-b border-gray-100">
                        ...
                        <div className="text-right">
                          <span className="block font-medium text-gray-900">
                            {bookingData.selectedDays.length} sessions
                          </span>
                    ...
                    {(bookingData.recurrenceWeeks || 1) * (bookingData.selectedDays?.length || 1) >
                      1 && (
                      <div className="flex justify-between py-3 border-b border-gray-100 bg-gray-50 -mx-6 px-6">
                        ...
                        <span className="font-medium text-gray-900">
                          {(bookingData.recurrenceWeeks || 1) *
                            (bookingData.selectedDays?.length || 1)}
                        </span>
                      </div>
                    )}
                    {discountAmount !== 0 && (
                    ...
                    <div className="space-y-3">
                      <div className="flex justify-between py-3 border-b border-gray-100">
                        ...
                        <span className="font-medium text-gray-900 capitalize">
                          {bookingData.queueSessionData.mode}
                        </span>
                      </div>
                      {(bookingData.queueSessionData.minSkillLevel != null ||
                        bookingData.queueSessionData.maxSkillLevel != null) && (
                        <div className="flex justify-between py-3 border-b border-gray-100">
                      ...
                      <div className="flex justify-between py-3 border-b border-gray-100">
                        ...
                        <span className="font-medium text-gray-900 capitalize">
                          {bookingData.queueSessionData.gameFormat}
                        </span>
                      </div>
                      <div className="flex justify-between py-3 border-b border-gray-100">
                        <span className="text-gray-600">Max Players:</span>
                        <span className="font-medium text-gray-900">
                          {bookingData.queueSessionData.maxPlayers}
                        </span>
                      </div>
                      <div className="flex justify-between py-3 border-b border-gray-100">
                        <span className="text-gray-600">Cost Per Game:</span>
                        <span className="font-medium text-gray-900">
                          ₱{bookingData.queueSessionData.costPerGame}
                        </span>
                      </div>
                      <div className="flex justify-between py-3 border-b border-gray-100">
                        <span className="text-gray-600">Visibility:</span>
                        <span
                          className={`font-medium ${bookingData.queueSessionData.isPublic ? 'text-green-600' : 'text-gray-600'}`}
                        >
                      ...
                      <div className="flex justify-between py-3">
                        ...
                        <span className="font-medium text-gray-900">
                          {bookingData.queueSessionData.joinWindowHours != null
            ...
            {currentStep === 'payment' && (
              <>
                ...
                <div className="bg-white border border-gray-200 rounded-xl p-6">
                  ...
                  {isMultiCourt && (
                    <div className="mb-3 p-3 bg-amber-50 border border-amber-200 rounded-lg text-sm text-amber-800">
                      Showing aggregate totals for {effectiveCart.length} cart items. Per-item
                      pricing appears in Booking Summary.
                    </div>
                  )}
                  <div className="space-y-3">
            ...
            {/* Step 4: Processing */}
            {currentStep === 'processing' && <PaymentProcessing />}
          </div>
