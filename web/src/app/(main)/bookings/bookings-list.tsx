...
export function BookingsList({ initialBookings }: BookingsListProps) {
  ...
  const getUnpaidEarlierDays = (booking: Booking): number[] => {
    ...
    for (const b of sameGroupBookings) {
      ...
      if (bIndex < currentIndex) {
        ...
        const isPaidStatus =
          ['confirmed', 'completed', 'ongoing'].includes(b.status) && isFullyPaid;
  ...
  const handleResumePayment = async (
    booking: Booking,
    paymentMethod: 'gcash' | 'paymaya' = 'gcash'
  ) => {
  ...
  const awaitingPayment = filteredBookings.filter((b) => {
    const isFullyPaid = b.amount_paid >= b.total_amount;
    return (
      ['pending_payment', 'pending', 'partially_paid'].includes(b.status) ||
      (b.status === 'confirmed' && !isFullyPaid)
    );
  }).length;
  const summaryBookings = [...bookings].sort(
  ...
  return (
    <div>
      ...
      <Tabs defaultValue="upcoming" className="w-full" onValueChange={setActiveTab}>
        ...
        <TabsContent value="upcoming" className="space-y-6">
          ...
          <div className="grid grid-cols-3 gap-2 sm:gap-3">
            {/* Total Bookings */}
            <div
              className="stat-glass stat-glass-teal relative overflow-hidden rounded-xl p-2.5 sm:rounded-2xl sm:p-4 border border-white/20 transition-transform duration-300 hover:-translate-y-1 hover:scale-[1.02]"
              style={{
                backgroundImage: [
            ...
            >
              ...
              <div className="relative z-10 flex items-center gap-2 sm:gap-3">
                <div
                  className="hidden w-10 h-10 rounded-full sm:flex items-center justify-center border border-white/30"
                  style={{
                ...
                <div>
                  <p className="text-[10px] sm:text-xs leading-tight font-medium text-white/75">
                    Total Bookings
                  </p>
                  <p
                    className="text-lg sm:text-2xl leading-none sm:leading-normal font-bold text-white"
            ...
            {/* Awaiting Payment */}
            <div
              className="stat-glass stat-glass-amber relative overflow-hidden rounded-xl p-2.5 sm:rounded-2xl sm:p-4 border border-white/20 transition-transform duration-300 hover:-translate-y-1 hover:scale-[1.02]"
              style={{
                backgroundImage: [
            ...
            {/* Confirmed / Paid */}
            <div
              className="stat-glass stat-glass-emerald relative overflow-hidden rounded-xl p-2.5 sm:rounded-2xl sm:p-4 border border-white/20 transition-transform duration-300 hover:-translate-y-1 hover:scale-[1.02]"
              style={{
                backgroundImage: [
            ...
            <div className="rounded-2xl border border-primary/15 bg-gradient-to-br from-primary/5 via-white to-teal-50 p-10 text-center">
              <div className="max-w-sm mx-auto">
                ...
                <Link href="/courts">
                  <Button
                    size="lg"
                    className="bg-primary hover:bg-primary/90 rounded-xl shadow-md shadow-primary/20 px-8"
                  >
                    <svg
            ...
            <div className="rounded-2xl border border-gray-200 bg-gradient-to-br from-gray-50 via-white to-gray-50 p-10 text-center">
              <div className="max-w-sm mx-auto">
                ...
                <p className="text-sm text-gray-500 leading-relaxed">
                  Your past bookings and completed sessions will appear here.
                </p>
              </div>
            </div>
          ) : (
          ...
          {groupedBookings.length === 0 ? (
            <div className="rounded-2xl border border-gray-200 bg-gradient-to-br from-gray-50 via-white to-gray-50 p-10 text-center">
              <div className="max-w-sm mx-auto">
                ...
                <p className="text-sm text-gray-500 leading-relaxed">
                  Your refund requests and processed refunds will appear here.
                </p>
              </div>
            </div>
          ) : (
          ...
          <div className="space-y-4">
            {bookings.length === 0 ? (
              ...
              <Card className="sqc-card">
                <CardHeader
                  className="sqc-header px-6 py-5 border-b border-primary/5 pb-5"
                  style={{
                ...
                >
                  ...
                  <CardTitle className="relative z-10 text-xl font-bold text-white tracking-tight drop-shadow-sm">
                    Booking Summary
                  </CardTitle>
                </CardHeader>
                <CardContent className="p-0 bg-white">
                  <div className="md:hidden p-3 space-y-2">
                    {summaryBookings.map((booking) => {
                      const paymentMethod =
                        booking.metadata?.intended_payment_method ||
                    ...
                    <Table className="border-0">
                      <TableHeader className="border-0">
                        <TableRow className="bg-gray-50/50 hover:bg-gray-50/50 border-b border-gray-100">
                          <TableHead className="font-semibold text-gray-600 pl-6 w-[40%] text-xs uppercase tracking-wider py-4">
                            Venue & Court
                          </TableHead>
                          <TableHead className="font-semibold text-gray-600 text-xs uppercase tracking-wider py-4">
                            Date & Time
                          </TableHead>
                          <TableHead className="font-semibold text-gray-600 text-right pr-6 text-xs uppercase tracking-wider py-4">
                            Payment
                          </TableHead>
                        </TableRow>
                      </TableHeader>
                      <TableBody className="border-0">
                        {summaryBookings.map((booking) => {
                          ...
                          return (
                            <TableRow
                              key={booking.id}
                              className="border-b border-gray-100/50 last:border-0 hover:bg-teal-50/40 transition-all duration-300 cursor-pointer group"
                              onClick={() => handleSelectBooking(booking)}
                            >
        ...
        <DialogContent className="inset-0 translate-x-0 translate-y-0 data-[state=open]:slide-in-from-left-0 data-[state=open]:slide-in-from-top-0 data-[state=closed]:slide-out-to-left-0 data-[state=closed]:slide-out-to-top-0 w-screen h-[100dvh] max-w-none max-h-none p-0 overflow-hidden rounded-none border-0 shadow-none sm:inset-auto sm:left-1/2 sm:top-1/2 sm:-translate-x-1/2 sm:-translate-y-1/2 sm:data-[state=open]:slide-in-from-left-1/2 sm:data-[state=open]:slide-in-from-top-[48%] sm:data-[state=closed]:slide-out-to-left-1/2 sm:data-[state=closed]:slide-out-to-top-[48%] sm:max-w-lg sm:max-h-[90vh] sm:flex sm:flex-col sm:!rounded-2xl sm:border sm:border-primary/15 sm:shadow-xl sm:shadow-primary/10">
          ...
          {selectedBooking && (
            <BookingCard
              ...
              onCancelBooking={(b, target) => {
                setSelectedBooking(null);
                handleCancelBooking(b, target);
              }}
              onRefundBooking={(b) => {
                setSelectedBooking(null);
      ...
      <Dialog
        open={!!outOfOrderWarning}
        onOpenChange={(open) => !open && setOutOfOrderWarning(null)}
      >
        <DialogContent className="!rounded-2xl border-amber-200 max-w-md p-6">
          <DialogTitle className="flex items-center gap-2 text-amber-700 text-lg font-semibold">
            ...
            Unpaid Earlier Booking
            {outOfOrderWarning && outOfOrderWarning.unpaidDays.length > 1 ? 's' : ''}
          </DialogTitle>
          <div className="space-y-3 mt-2">
            <p className="text-sm text-gray-600">
              ...
              <span className="font-semibold text-gray-900">
                Day{' '}
                {outOfOrderWarning
                  ? (outOfOrderWarning.booking.metadata?.recurrence_index ??
                      outOfOrderWarning.booking.metadata?.week_index ??
                      0) + 1
                  : ''}
              </span>{' '}
              of your booking, but{' '}
              ...
              {outOfOrderWarning && outOfOrderWarning.unpaidDays.length > 1 ? 'are' : 'is'} still
              unpaid.
            </p>
