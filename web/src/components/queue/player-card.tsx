...
export function PlayerCard({ player, isCurrentUser }: PlayerCardProps) {
  const profileHref = isCurrentUser ? '/profile' : `/profile/${player.userId}`
  ...
  return (
    ...
    >
      <div
        className={`
          relative bg-white border rounded-xl p-4 transition-all duration-300 cursor-pointer group
          ${isCurrentUser
      ...
      >
        ...
        <div className="flex items-center gap-3">
          ...
          {!isCurrentUser && (
            <ChevronRight className="w-5 h-5 text-gray-300 group-hover:text-primary/60 transition-colors flex-shrink-0" />
          )}
        </div>
      </div>
    </Link>
