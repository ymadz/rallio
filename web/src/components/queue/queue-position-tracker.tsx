...
export function QueuePositionTracker({
  ...
  const badgeIcon = status === 'waiting'
  ...
  return (
    <>
      ...
      {/* Status Badge */}
      <div className={`inline-flex items-center gap-1.5 px-3.5 py-1.5 rounded-full border backdrop-blur-sm mb-4 ${statusColors[status]}`}>
        {badgeIcon}
        {status === 'waiting' && (
        ...
        <span className="font-semibold text-xs uppercase tracking-wider">{statusLabels[status]}</span>
      </div>
