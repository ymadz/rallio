...
export function PromoCodeInput() {
  ...
  const getItemBasePrice = useCallback(
    (item: (typeof effectiveCart)[number]) => {
  ...
  const getVenueBasePrice = useCallback(
    (venueId: string) => {
  ...
  const recalculateDiscounts = useCallback(
    async (promoCodeStr?: string, targetVenueId?: string) => {
      ...
      if (unifiedResult.success) {
        ...
        const venueTotal = venueDiscounts.reduce(
          (sum, d) => (d.isIncrease ? sum - d.amount : sum + d.amount),
          0
        );
  ...
  const discountRecalcKey = useMemo(() => {
    ...
    const cartSignature = effectiveCart
      .map(
        (item) =>
          `${item.courtId}|${new Date(item.date).toISOString()}|${item.startTime}|${item.endTime}|${item.recurrenceWeeks || 1}|${(item.selectedDays || []).join(',')}`
      )
      .join('||');
  ...
  if (promoCode && promoDiscountAmount > 0) {
    const promoVenueName = effectiveCart.find(
      (item) => item.venueId === promoTargetVenueId
    )?.venueName;
    return (
      <div className="bg-white border border-gray-200 rounded-xl p-5">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-sm font-medium text-gray-900">{promoCode.toUpperCase()} applied</p>
  ...
  const handleApply = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      const uniqueVenueIds = Array.from(
        new Set(effectiveCart.map((item) => item.venueId).filter(Boolean))
      );
