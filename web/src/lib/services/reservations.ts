...
export async function createReservation(
  ...
  console.log(
    `[createReservation] Starting ${isRecurring ? 'recurring' : 'single'} booking check. Weeks: ${recurrenceWeeks}, Days: ${selectedDays.join(',')}`
  );
  ...
  for (let i = 0; i < targetSlots.length - 1; i++) {
    ...
    if (current.end.getTime() > next.start.getTime()) {
      return {
        success: false,
        error: 'Booking Request Invalid: Generated slots overlap with each other.',
      };
    }
  }
  ...
  for (const slot of targetSlots) {
    ...
    const realConflicts =
      reservationConflicts.data?.filter((conflict) => {
        return ['pending_payment', 'partially_paid', 'confirmed', 'ongoing'].includes(
          conflict.status
        );
      }) || [];
  ...
  if (!bookingId) {
    ...
    if (bookingError || !newBooking) {
      ...
      return {
        success: false,
        error: `Failed to initialize booking: ${bookingError?.message || 'Unknown error'}`,
      };
    }
  ...
  const rawDownPaymentPercentage =
  ...
  const downPaymentPercentage = Number.isFinite(parsedDownPaymentPercentage)
    ...
    : 20;
  const minimumDownPaymentPerSlot =
    Math.round(((courtAmountPerSlot * downPaymentPercentage) / 100) * 100) / 100;
  ...
  const downPaymentAmount = shouldRequireDownPayment
    ? Math.round(
        (hasCustomDownPayment
          ? Math.min(
              Math.max(customDownPaymentPerSlot!, minimumDownPaymentPerSlot),
              perInstanceAmount
            )
          : minimumDownPaymentPerSlot) * 100
      ) / 100
    : undefined;

  const isCustomDownPayment =
    shouldRequireDownPayment &&
    hasCustomDownPayment &&
    downPaymentAmount !== minimumDownPaymentPerSlot;
  ...
  for (let i = 0; i < finalSlots.length; i++) {
    ...
    const { data: newRes, error } = await supabase
      ...
      .insert({
        ...
        metadata: {
          ...
          cash_payment_option:
            data.paymentMethod === 'cash' ? (data.cashPaymentOption ?? 'downpayment') : undefined,
    ...
    if (error || !newRes) {
      ...
      if (createdReservationIds.length > 0) {
        console.log(
          `🔄 Rolling back ${createdReservationIds.length} previously created reservations...`
        );
      ...
      await supabase
        ...
        .update({
          status: 'cancelled',
          metadata: { cancellation_reason: 'Failed to create all reservation slots' },
        })
        .eq('id', bookingId);
  ...
  return {
    ...
    downPaymentRequired:
      shouldRequireDownPayment && downPaymentAmount !== undefined && downPaymentAmount > 0,
